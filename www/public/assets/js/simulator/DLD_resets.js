...
$(function () {
    $.widget("plc.resets", {
        ...
        _create: function () {
            ...
            icon.attr("src", "/public/assets/icon/simulator/RES_LDL.png");
            node.append(icon);
                ...
                items: {
                    1: {name: "Rename", icon: "edit",
                        callback: function (key, options) {
                            $($(this).parent()).resets("input_name");
                        }
                    },
                    2: {name: "Delete", icon: "delete",
                        callback: function (key, options) {
                            var delete_line = $($(this).parent()).resets("option", "line");
                            var delete_offset = $($(this).parent()).resets("option", "offset");

                            deleteElement($($(this).parent()).resets("option", "LDL"), delete_line, delete_offset);
                    ...
                    3: {name: "Mark", icon: "quit",
                        callback: function (key, options) {
                            $($(this).parent()).resets("activate");
                        }
                    },
                    4: {name: "Unmark", icon: "quit",
                        callback: function (key, options) {
                            $($(this).parent()).resets("deactivate");
                        }
                    }
                }
            });
            this.element.droppable({
                ...
                drop: function( event, ui ) {
                    ...
                    $(this).resets("update_name",new_ID);
                }
            });
        },
        ...
        update_name: function (name) {
            ...
            var new_reset_name = get_reset_counter_name(new_ID);
            this.options.name = new_reset_name;

            var new_tag_name = get_tag_name_from_reset(new_ID);
            this.option.tag_name = new_tag_name;
            ...
            for(var i=0; i<counter_name_list.length; i++){
                if (new_tag_name.toUpperCase() == counter_name_list[i]){
                    registered = 1;
                }
            }
